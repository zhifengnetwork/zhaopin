...
class Member extends Common
{
    ...
    {
        $uid = input('id/d');
        $regtype = input('regtype/d');
        $member = MemberModel::get(['id' => $uid, 'regtype' => $regtype]);
        ...
        if ($regtype == 3) {
            $data = Db::name('member')->alias('m')
                ->field('m.id as mid,p.*,m.balance,m.mobile,m.createtime,m.content')
                ->join('person p', 'm.id=p.user_id', 'LEFT')
                ->where(['m.id' => $uid])
                ->find();
        } else if ($regtype == 1 || $regtype == 2) {
            $data = Db::name('member')->alias('m')
                ->field('m.id as mid,c.*,m.balance,m.content')
                ->join('company c', 'm.id=c.user_id', 'LEFT')
                ->where(['m.id' => $uid])
                ->find();
        }
        $this->assign('data', $data);
        $this->assign('meta_title', ($regtype == 3 ? '个人' : ($regtype == 2 ? '第三方' : '公司')) . '详情');
        return $this->fetch($regtype == 3 ? 'company/person_detail' : 'company/company_detail');
    }
