...
class AopClient {
	...
	//网关
	public $gatewayUrl = "https://openapi.alipayaop.com/gateway.do";
	//返回数据格式
	public $format = "json";
	//api版本
	...
	protected $alipaySdkVersion = "alipayaop-sdk-php-20180705";
...
/** 
	...
	public function encryptAndSign($bizContent, $rsaPublicKeyPem, $rsaPrivateKeyPem, $charset, $isEncrypt, $isSign, $signType='RSA') {
		...
		if ($isEncrypt && $isSign) {
			...
			$response = "<?xml version=\"1.0\" encoding=\"$charset\"?><alipayaop><response>$encrypted</response><encryption_type>RSA</encryption_type><sign>$sign</sign><sign_type>$signType</sign_type></alipayaop>";
			return $response;
		}
		// 加密，不签名
		if ($isEncrypt && (!$isSign)) {
			$encrypted = $this->rsaEncrypt($bizContent, $rsaPublicKeyPem, $charset);
			$response = "<?xml version=\"1.0\" encoding=\"$charset\"?><alipayaop><response>$encrypted</response><encryption_type>$signType</encryption_type></alipayaop>";
			return $response;
		}
		// 不加密，但签名
		if ((!$isEncrypt) && $isSign) {
			$sign = $this->sign($bizContent, $signType);
			$response = "<?xml version=\"1.0\" encoding=\"$charset\"?><alipayaop><response>$bizContent</response><sign>$sign</sign><sign_type>$signType</sign_type></alipayaop>";
			return $response;
		}
		// 不加密，不签名
